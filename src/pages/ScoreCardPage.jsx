...
import { Share2, Trophy, Flame, Target, Droplets, Star, Copy, Check, Zap, ChevronRight } from 'lucide-react';
    ...
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <svg width={size} height={size} className="-rotate-90">
        ...
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={rank.color} strokeWidth={sw} strokeLinecap="round"
          strokeDasharray={c} strokeDashoffset={on ? c - (score / 100) * c : c}
...
function StatPill({ icon: emoji, val, unit, label, color, delay }) {
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-11 h-11 rounded-2xl bg-gradient-to-br ${rank.bg} flex items-center justify-center shadow-lg`}
                    style={{ boxShadow: `0 4px 12px ${rank.color}25` }}>
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-muted-foreground tracking-wide">30-DAY HYDRATION REPORT</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  {selectedDayData && (
                    <p className="text-[9px] font-semibold stagger-in" style={{ color: getScoreRank(selectedDayData.score).color }}>
                      Day {14 - selectedDay}: {selectedDayData.score}pts · {selectedDayData.total}ml
                    </p>
                  )}
                </div>
          ...
          <div className="glass-card p-4 slide-up" style={{ animationDelay: '300ms' }}>
            <div className="flex items-center justify-between mb-2">
              ...
              {(() => {
                ...
                const nextRank = getScoreRank(next);
                return <span className="text-[9px] font-bold" style={{ color: nextRank.color }}>{nextRank.emoji} {nextRank.rank} — {nextRank.label}</span>;
              })()}
            </div>
            {(() => {
              const thresholds = [20, 40, 60, 80, 95];
              ...
              return (
                <div>
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${progress}%`, background: nextRank.color, transitionTimingFunction: 'cubic-bezier(0.23,1,0.32,1)' }} />
                  </div>
                  <div className="flex justify-between mt-1">
                    <span className="text-[9px] text-muted-foreground">{data.avgScore} pts</span>
                    <span className="text-[9px] font-medium" style={{ color: nextRank.color }}>{next} pts needed</span>
                  </div>
                </div>
              );
            })()}
        ...
        <div className="glass-card p-4 space-y-2 slide-up" style={{ animationDelay: '400ms' }}>
          ...
          <div className="space-y-1">
            {[
              ...
              return (
                <div key={r.rank} className={`flex items-center gap-2.5 p-2 rounded-lg transition-colors ${isCurrentRank ? 'bg-muted' : ''}`}>
                  ...
                  {isCurrentRank && <ChevronRight size={10} style={{ color: r.color }} />}
                </div>
              );
            })}
          </div>
